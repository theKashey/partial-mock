...
describe('partial mocks', () => {
  ...
  it('htmlelement mocks', () => {
    const element = partialMock<HTMLElement>({ clientHeight: 50 });
    expect(element.clientHeight).toBe(50);
    expect(() => element.clientWidth).toThrow();
  });
  ...
  it('does not span over native methods', () => {
    const mock = partialMock<[{ a: number }]>([{ a: 1 }]).map(
      (x) =>
        ({ ...x, y: 2 } as {
          a: number;
          y: number;
    ...
    // should pass
    expect(mock[0].a).toBe(1);
    // should pass
    expect(mock[0].b).toBe(undefined);
  });
});
