...
const getReal = (entity: any) => (entity && entity[A_REAL_INSTANCE]) ?? entity;

const unproxify = (ref: any) => {
  const target = getReal(ref);

  const followProp = (value: any): any => {
    if (
      typeof value === 'object' &&
      // value.then &&
      value instanceof Promise
    ) {
      return value.then((res) => followProp(res));
    }

    if (Array.isArray(value) || (typeof value === 'object' && value) || typeof value === 'function') {
...
const proxify = (refs: RefsMap, target: any, extras: any, suffix: string, report: (name: string) => void) => {
  ...
  const followProp = (value: any, name: string, extras = value): any => {
    ...
    if (
      ...
      // skip build in methods like `.map`
      !Object.getPrototypeOf(target).hasOwnProperty(name)
    ) {
      return proxify(refs, value, extras, name, report);
    }
...
const mockRegistry = new WeakMap<
  any,
  {
    getUsage(): Set<string>;
    resetUsage(): void;
